...
"""
    ...
    Crea todos los operadores de spin tipo Stevens para un dado J=dj. El espacio es de dimension 2J+1.
    Tambien crea Jx,Jy,Jz y todos los de campo cristalino hasta orden 6.
    La salida es una named tuple con todos los operadores.
    Calculates the spin operators for a system with spin `dj`.

        # Arguments
        - `dj::Number`: the spin of the system.
        
        # Returns a named tuple with the following fields:
        - `Jx::Matrix`: the x-component of the spin operator.
        - `Jy::Matrix`: the y-component of the spin operator.
        - `Jz::Matrix`: the z-component of the spin operator.
        - `O20::Matrix`: the operator O20.
        - `O22::Matrix`: the operator O22.
        - `O2e::Matrix`: the operator O2e.
        - `O40::Matrix`: the operator O40.
        - `O42::Matrix`: the operator O42.
        - `O43::Matrix`: the operator O43.
        - `O44::Matrix`: the operator O44.
        - `O4M3::Matrix`: the operator O4M3.
        - `O4M4::Matrix`: the operator O4M4.
        - `O60::Matrix`: the operator O60.
        - `O62::Matrix`: the operator O62.
        - `O63::Matrix`: the operator O63.
        - `O6M3::Matrix`: the operator O6M3.
        - `O64::Matrix`: the operator O64.
        - `O66::Matrix`: the operator O66.
        - `O6M6::Matrix`: the operator O6M6.
...
function OperadoresDeSpin(dj::Number)
    ...
    return (Jx = Jx, Jy = Jy,Jz = Jz, 
            ...
            O40 = O40, O41 = O41, O42 = O42, O43 = O43, O44 = O44, O4M3 = O4M3, O4M4 = O4M4,
...
function VecinosConVec(Posiciones, celula, VecVecinos)
    ...
    for origen in eachrow(Posiciones)
        ...
        for desplazamiento in eachrow(VecVecinos)
            #println("desplazamiento :", desplazamiento, "\t")
            if origen[5] == desplazamiento[1]
                #print("desplazamiento2 :", desplazamiento, "\t")
        ...
        push!(TVecinos, Vecinos) # El "sort(Vecinos))" crea problemas con Sunny
        #break
    end
    return TVecinos
end;
...
"""
    ...
    Entradas:
        TodoEnergia: Energias de los estados del sitio
        T: Temperatura
    Salidas (named tuple):
        Z: Funcion de particion
        S: Entropia
        Pi: Probabilidades de cada estado
"""
...
"""
    ValorMedioTempOp(Estados, FunPart, Operador)
    calcula el valor medio termodinamico del Operador.
    Entradas:
        Estados: Estados del sitio
        FunPart: Funcion de particion
        Operador: Operador
    Salidas:
        Valor medio termodinamico del operador.
...
"""
    NuevoSpinSitio(Interac,  TablaValMedSpines, Temp, TablaValorMedioO20, Bmag, SpinOp, HEff::Function)
    Calcula el nuevo valor medio del spin de un sitio. Tambien calcula la funcion de particion y el valor medio del O20.
    Entradas:
        Interac: Interaccion del sitio
        TablaValMedSpines: Tabla de valores medios de los spines
        Temp: Temperatura
        TablaValorMedioO20: Tabla de valores medios de los O20
        Bmag: Campo magnetico
        SpinOp: Operadores de spin
        HEff: Funcion que calcula el hamiltoniano efectivo del sitio
    Salidas:
        NuevoSpin: Nuevo valor medio del spin
        FunPart: Funcion de particion
        NuevoO20: Nuevo valor medio del O20
"""
...
"""
    ActualizoSpines(TablaValMedSpines, SitioEInt, Temp, TablaValorMedioO20, Bmag, SpinesOp, HEff::Function)
    Actualiza los valores medios de los spines de todos los sitios.
    Entradas:
        TablaValMedSpines: Tabla de valores medios de los spines
        SitioEInt: Sitios y sus interacciones
        Temp: Temperatura
        TablaValorMedioO20: Tabla de valores medios de los O20
        Bmag: Campo magnetico
        SpinesOp: Operadores de spin
        HEff: Funcion que calcula el hamiltoniano efectivo del sitio
    Salidas:
        NuevosSpines: Nuevos valores medios de los spines
        Entropia: Entropia por sitio
        NuevosO20: Nuevos valores medios de los O20
"""
...
"""
    Convergencia(TablaValMedSpines, SitioEInt, ItMax, CutOff, Temp, TablaValorMedioO20, Bmag, SpinesOp, HEff::Function)
    Calcula los valores medios de los spines de todos los sitios hasta que converja.
    Entradas:
        TablaValMedSpines: Tabla de valores medios de los spines
        SitioEInt: Sitios y sus interacciones
        ItMax: Maximo numero de iteraciones
        CutOff: Corte para la norma de la diferencia entre dos iteraciones
        Temp: Temperatura
        TablaValorMedioO20: Tabla de valores medios de los O20
        Bmag: Campo magnetico
        SpinesOp: Operadores de spin
...
"""
    Magnetizacion(Campo, Spines)
    Calcula la magnetizacion de un conjunto de spines.
    Entradas:
        Campo: Campo magnetico
        Spines: Spines
    Salidas:
...
function Magnetizacion(Campo, Spines)
   ...
   if MagnitudCampo > 0
          ...
          Magnetizacion = - 1. / TotalSpines * norm(sum( Spines[i , :] for i in  1 : TotalSpines))
   end
...
"""
    MedO20(O20s)
    Calcula el valor medio del O20.
    Entradas:
        O20s: O20s
    Salidas:
        ValorMedioO20: Valor medio del O20
"""
...
"""
    BarroCampoTemp(Campos, direccionB, Temps, TablaValMedSpines, TablaValorMedioO20, SitioEInt, SpinesOp, HEff::Function)
    Calcula la magnetizacion y el valor medio del O20 para un conjunto de campos y temperaturas.
    Entradas:
        Campos: Campos magneticos
        direccionB: Direccion del campo magnetico
        Temps: Temperaturas
        TablaValMedSpines: Tabla de valores medios de los spines
        TablaValorMedioO20: Tabla de valores medios de los O20
        SitioEInt: Sitios y sus interacciones
        ...
        HEff: Funcion que calcula el hamiltoniano efectivo del sitio
    Salidas (named tuple):
        PropTermo: Propiedades termodinamicas (Temperatura, Campo, Magnetizacion, Entropia, Valor medio del O20)
        UltConfSpin: Ultima configuracion de los spines
        UltConfO20: Ultima configuracion del O20
"""
